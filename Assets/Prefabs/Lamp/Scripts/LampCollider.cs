...
public class LampCollider : MonoBehaviour
{
	...
	void Start()
	{
		...
		TurnOn = lamp.GetComponent<Lamp>().TurnOn;
		...
		if (playerEntered)
		{
			...
			if (Physics.Raycast(rayOrigin, fpsCam.transform.forward, out hit, reachRange, rayLayerMask))
			{
				...
				if (moveableObject != null)     //hit object must have MoveableDraw script attached
				{
					...
					msg = getGuiMsg(TurnOn);

					if (Input.GetKeyUp(KeyCode.E) || Input.GetButtonDown("Fire1"))
					{
						if (TurnOn)
						{
							lamp.GetComponent<Lamp>().TurnOn = false;
							TurnOn = false;
						}
						else
						{
							lamp.GetComponent<Lamp>().TurnOn = true;
							TurnOn = true;
						}
						msg = getGuiMsg(!TurnOn);
					}
	...
	private string getGuiMsg(bool isOpen)
	{
		...
		if (isOpen)
		{
			rtnVal = "Press E/Fire1 to Turn off";
		}
		else
		{
			rtnVal = "Press E/Fire1 to Turn on";
		}
